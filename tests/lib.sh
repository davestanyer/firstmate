...
# fm_fake_crash_injector drops the shim a fake uses to crash the process under
# test deterministically. fm_fake_version_tool drops a stub for a tool whose
# installed version bootstrap gates, so a fixture cannot be reported as an
...
while [ "$waited" -lt 600 ]; do
  # MSYS ps rejects -o entirely, so field selection reads empty there and this
  # loop would report the target gone while it is still running - the exact
  # race the shim exists to close. The Cygwin procfs answers for MSYS pids. The
  # comm field is parenthesized and may contain spaces, so the state letter is
  # taken as the first field after the last ") ", not by a positional cut.
