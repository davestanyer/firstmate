...
A profile `provider` optionally names the quota-axi provider family whose rows apply to that profile; bootstrap validates it only when present, preserving existing provider-less profiles when typed resolution is off. The opted-in resolver requires it for harnesses that serve several families (`pi`, `pi-signed`, `omp`, `opencode`) and returns an actionable configuration error when one omits it. A profile `floor` contains only `scope` and `min_percent`, always uses that profile's provider, and makes that one candidate ineligible below `min_percent` on the named scope. An absent or unknown named row also makes the candidate unrankable and is reported as an unverifiable floor, not as a known shortfall.
